...
int em_metrics_t::handle_ap_metrics_query(unsigned char *buff, unsigned int len)
{
    ...
    memcpy(bss, tlv->value, sizeof(mac_address_t));

    send_ap_metrics_response(bss);
    set_state(em_state_agent_configured);

    return 0;
}

int em_metrics_t::handle_ap_metrics_resp(unsigned char *buff, unsigned int len)
{
    em_tlv_t *tlv;
    int tmp_len, ret = 0;
...
int em_metrics_t::send_ap_metrics_msg(mac_address_t ap_mac)
{
    ...
    sz = create_ap_radio_identifier_tlv(tlv->value);
    tlv->len = htons(sz);
...
int em_metrics_t::send_ap_metrics_response(mac_address_t ap_mac)
{
    unsigned char buff[MAX_EM_BUFF_SZ];
    ...
    dm_easy_mesh_t::macbytes_to_string(ap_mac, mac_str);
    ...
    sz = create_ap_ext_metrics_tlv(tlv->value,ap_mac);
    tlv->len =  htons(sz);
    ...
    tlv = (em_tlv_t *)tmp;
    tlv->type = em_tlv_type_assoc_sta_link_metric;
    sz = create_radio_metrics_tlv(tlv->value,...); //TODO:determine radio
    tlv->len =  htons(sz);
    ...
    tlv = (em_tlv_t *)tmp;
    tlv->type = em_tlv_type_assoc_sta_link_metric;
    sz = create_assoc_sta_traffic_metrics_tlv(tlv->value,...);//TODO: determine station
    tlv->len =  htons(sz);
    ...
    if (em_msg_t(em_msg_type_ap_metrics_rsp, em_profile_type_3, buff, len).validate(errors) == 0) {
        printf("%s:%d: AP Link Metrics validation failed for %s\n", __func__, __LINE__, mac_str);
        return -1;
    }
    ...
    printf("%s:%d: AP Link Metrics for AP %s sent successfully\n", __func__, __LINE__, mac_str);
...
short em_metrics_t::create_ap_metrics_query_tlv(unsigned char *buff)
{
    ...
    ap_metrics_query->num_bssids = dm->m_num_bss;//Question: I'm not sure what should determine num_bssids
...
short em_metrics_t::create_ap_radio_identifier_tlv(unsigned char *buff)
{
    short len = 0;
    unsigned int radios;
    unsigned int i;
    ...
    radios = dm->m_num_radios;//QUESTION: should it be determined like that or there is difference how controller and agent see stuff?

    for(i = 0; i < radios; i++)
    {
        memcpy(ruid, dm->m_radio[i].get_radio_info()->id.mac,sizeof(em_radio_id_t));
