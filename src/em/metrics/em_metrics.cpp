...
int em_metrics_t::handle_ap_metrics_query(unsigned char *buff, unsigned int len)
{
    ...
    send_ap_metrics_response(bss); //QUESTION: if querry contains multiple bssids then it should 
...
int em_metrics_t::send_ap_metrics_msg(mac_address_t ap_mac)
{
    ...
    for (i = 0; i < dm->m_num_radios; i++) //QUESTION: according to spec I should have zero or more TLVs depending on radios, is this way of quering number of radions proper?
    {
        tlv = (em_tlv_t *)tmp;
...
int em_metrics_t::send_ap_metrics_response(mac_address_t ap_mac)
{
    ...
    tlv = (em_tlv_t *)tmp;
    tlv->type = em_tlv_type_assoc_sta_link_metric;
    sz = create_radio_metrics_tlv(tlv->value,...); //QUESTION:determine radio? from bss_info?
    tlv->len =  htons(sz);
...
short em_metrics_t::create_ap_metrics_query_tlv(unsigned char *buff)
{
    ...
    ap_metrics_query->num_bssids = dm->m_num_bss;//Question: Is this way of determining number of bssids proper?
